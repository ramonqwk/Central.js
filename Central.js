(function () {
    ...
    const log = (msg, isError = false) => {
        ...
        line.textContent = (isError ? "[ERRO] " : "> ") + msg;
        line.style.color = isError ? "#f33" : "#0f0";
        logBox.appendChild(line);
        logBox.scrollTop = logBox.scrollHeight;
    };

    // Verificar se está na página de lição
    ...
    if (!lesson_regex.test(window.location.href)) {
        log("Esta página não é uma lição. Acesse uma lição para rodar o script.", true);
        return;
    }
    ...
    try {
        ...
        sessionData = JSON.parse(sessionStorage.getItem(`${platform}:iptvdashboard:state`));
        if (!sessionData || !sessionData.auth || !sessionData.room || !sessionData.auth.auth_token) {
            log("Dados de sessão incompletos. Faça login corretamente.", true);
            return;
        }
        ...
        const sendRequest = (method, url, data = null, callback) => {
            ...
            xhr.onerror = () => log(`Erro de rede: ${method} ${url}`, true);
        ...
        sendRequest("POST", `https://edusp-api.ip.tv/tms/task/${lessonId}/answer`, draftBody, draftRes => {
            ...
            sendRequest("GET", getUrl, null, getRes => {
                if (getRes.status !== 200) {
                    log("Erro ao buscar respostas.", true);
                    return;
                }
